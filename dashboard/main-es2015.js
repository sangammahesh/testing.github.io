...
let EmployeedetailComponent = class EmployeedetailComponent {
    constructor(route, employeeService, _router) {
        this.route = route;
    ...
    ngOnInit() {
        const param = this.route.snapshot.paramMap.get('id'); // taking id from URL ( we are getting in string format)
        if (param) {
            const id = +param; // converting in number
            this.getEmployee(id);
        }
    }
    getEmployee(id) {
...
let EmployeedetailsGuard = class EmployeedetailsGuard {
    constructor(router) {
        this.router = router;
    }
...
let EmployeefilterPipe = class EmployeefilterPipe {
    transform(value, searchTerm) {
        ...
        return value.filter(employee => employee.name.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1);
    }
};
...
let EmployeesPipe = class EmployeesPipe {
...
let EmployeesService = class EmployeesService {
    ...
    getEmployee(id) {
        return this.getEmpData().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])((employees) => employees.find(e => e.empid === id)));
    }
    handleError(error) {
...
let EmployeesComponent = class EmployeesComponent {
    constructor(empData) {
        ...
        this.errorMsg = '';
        empData.getEmpData().subscribe(data => {
            this.employees = data;
...
let EmployeesgaurdService = class EmployeesgaurdService {
    constructor() { }
};
...
let RemoveSpacePipe = class RemoveSpacePipe {
...
let StarratingPipe = class StarratingPipe {
    transform(value, args) {
